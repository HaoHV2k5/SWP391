  ...
  const products = useMemo(() => {
    return rawProducts
      .map((p) => {
        ...
        const brand = p.brand ?? p.vehicleBrand ?? p.manufacturer ?? '';
        const year = String(p.year ?? p.modelYear ?? p.vehicleInfo?.year ?? '');
        const type = p.type ?? p.category ?? p.vehicleType ?? '';
        ...
        return {
          ...
          vehicleInfo: { title, description, year },
          isActive,
        };
      })
